...
namespace PanasonicNZ.Common
{
    public class MAPI
    {
        ...
        int SendMail(string strSubject, string strBody, int how)
        {
            ...
            if (m_lastError > 1)
                throw new MAPIException(m_lastError, "MAPISendMail failed! " + GetLastError());
    ...
    {
        public MAPIException(int errorcode, string description)
           : this(errorcode, description, null)
        {
        }

        public MAPIException(int errorcode, string description, Exception innerException)
