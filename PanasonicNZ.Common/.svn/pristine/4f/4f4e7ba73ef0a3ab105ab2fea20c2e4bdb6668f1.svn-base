...
namespace PanasonicNZ.Common
{
    ...
    public class ShellInfo
    {
        ...
        public const float MB = KB * KB;
        public const float GB = KB * MB;
        ...
        private static ImageSource GetIcon(string fileName, SHGFI flags)
        {
            ...
            SHGetFileInfo(fileName, 0, ref shinfo, (uint)Marshal.SizeOf(shinfo), (uint)(SHGFI.Icon | flags));
        ...
        public static string GetFileTypeName(string fileName)
        {
            ...
            uint dwFileAttributes = (uint)FileAttributes.Normal;

            // Set the operation flags for retrieving the desired information
            uint uFlags = (uint)(SHGFI.UseFileAttributes | SHGFI.DisplayName | SHGFI.TypeName);
        ...
        public static string GetFileSizeText(long size)
        {
            ...
            if (size < GB)
                return $"{size / MB:n2} MB";

            return $"{size / GB:n2} GB";
        }

        public static void RunAssociatedApplication(string fullName)
        {
            Process.Start(fullName);
        }

        public static void RestartAsAdmin(params string[] args)
        {
            ProcessStartInfo psi = new ProcessStartInfo();
            psi.FileName = Assembly.GetEntryAssembly().Location;
            if (args == null || args.Length == 0)
                psi.Arguments = String.Join(" ", Environment.GetCommandLineArgs().Skip(1).Select(s => s.Contains(" ") ? $"\"{s}\"" : s));
            else
                psi.Arguments = String.Join(" ", args.Select(s => s.Contains(" ") ? $"\"{s}\"" : s));
        ...
        public static void ShowFileProperties(System.Windows.Forms.IWin32Window owner, string filename)
        {
            ...
            if (owner != null) info.hwnd = owner.Handle;
        ...
        public static bool ShowCredentialsDialog(string serverName, out NetworkCredential networkCredential)
        {
        ...
        public static ImageSource GetShieldIcon()
        {
            ...
            if (Environment.OSVersion.Version.Major >= 6)
            {
                ...
                shieldSource = Imaging.CreateBitmapSourceFromHIcon(sii.hIcon, Int32Rect.Empty, BitmapSizeOptions.FromEmptyOptions());
                DestroyIcon(sii.hIcon);
            }
            else
            {
                shieldSource = Imaging.CreateBitmapSourceFromHIcon(System.Drawing.SystemIcons.Shield.Handle, Int32Rect.Empty, BitmapSizeOptions.FromEmptyOptions());
            }
